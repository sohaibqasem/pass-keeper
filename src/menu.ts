...
async function handleRequest(selected: string, value?:string) {
...
export async function mainMenu(appname?:string) {
    if(appname) {
        handleRequest("Find a password by (App name)", appname);
        return
    }
